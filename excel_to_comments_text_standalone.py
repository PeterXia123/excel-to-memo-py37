...
def build_comments_text_from_dataframe(
    ...
    normalized = filter_to_breach_rows(normalized)
...
def filter_to_breach_rows(normalized: pd.DataFrame) -> pd.DataFrame:
    filtered = normalized.loc[
        normalized["status_raw"].map(
            lambda value: normalize_whitespace(value).lower() == "breach"
        )
    ].copy()
